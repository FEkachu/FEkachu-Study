...
변수의 선언과 초기화를 분리한 후, 선언만 코드의 최상단으로 옮기는 것으로 설명을 많이한다.

- 변수?
    
    변수는 하나의 값을 저장하기 위해 확보한 메모리 공간 자체, 혹은 메모리 공간을 식별하기 위해 붙인 이름을 말한다. 
    

### Javascript 변수 선언에 따른 동작

호이스팅을 이해하기 위해서는 javascript에서 변수 선언이 어떻게 이루어지고, 할당되는지를 알아야 한다.  

변수 선언과 할당의 순서는 다음과 같다.

> 1. 선언 단계 - 변수를 실행 컨텍스트의 변수 객체에 등록한다. 
> 
> 2. 초기화 단계 - 변수 객체에 등록된 변수를 위한 공간을 메모리에 확보한다. 이 때 변수는 undefined로 초기화된다.
>
...
자바스크립트에서 변수 선언은 **선언 → 초기화 단계**를 거쳐서 수행된다. 

**선언 단계**에서는 변수명을 등록해 자바스크립트 엔진에 변수의 존재를 알린다.

**초기화 단계**에서는 값을 저장하기 위한 메모리 공간을 확보하고, 암묵적으로 undefined를 할당해 초기화 한다. 이때 var와 let, const는 동작 방식이 다르다. 

var의 경우 위에서 설명한 것과 같이 **선언과 초기화가 동시**에 진행된다.

let, const는 **선언과 초기화가 분리**되어 이루어진다. 그렇기 때문에 스코프에 변수를 선언하기는 하지만, 선언문 부분에 도달해야 초기화가 이루어진다. 

### TDZ

이러한 사유로 Temporal Dead Zone이라는 구간이 생겨났다. let, const는 스코프에 변수를 선언하지만, 선언 부분에 도달하지 않으면 초기화가 이루어지지 않는다. 

그렇기 때문에 스코프의 시작부터 선언 부분까지는 해당 변수를 사용할 수 없는 구간이 생기게 되는데 이를 TDZ라고 부른다.

```jsx
function hello() {
	
	console.log('Here is TDZ!!');
	console.log(a); // error, 함수 시작부터 여기까지 TDZ!!
	let a;
...
[변수 선언 키워드 - let, const 그리고 var](https://study-ihl.tistory.com/161)
